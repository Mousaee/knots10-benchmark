...
MODEL_NAMES = {
    ...
    'transfg': 'TransFG', 'pmg': 'PMG', 'graph_fgvc': 'Graph-FGVC'
}
MODEL_ORDER = ['resnet18','resnet50','efficientnet_b0','vit','swin_t',
               'transfg','pmg','graph_fgvc']
# 8 colorblind-friendly colors
COLORS = ['#0072B2','#D55E00','#009E73','#CC79A7','#E69F00',
          '#56B4E9','#F0E442','#999999']
MARKERS = ['o','s','^','D','v','P','X','h']
# Model grouping: General vs FGVC-specialized
MODEL_GROUPS = {
    'resnet18': 'General', 'resnet50': 'General', 'efficientnet_b0': 'General',
    'vit': 'General', 'swin_t': 'General',
    'transfg': 'FGVC', 'pmg': 'FGVC', 'graph_fgvc': 'FGVC'
}
...
for k in MODEL_ORDER:
    ...
    if os.path.exists(fp):
        with open(fp) as f:
            data = json.load(f)
        # Normalize keys across different model JSON formats
        if 'per_class_report' in data and 'report' not in data:
            data['report'] = data['per_class_report']
        if 'best_val_acc' not in data and 'val_acc' in data:
            data['best_val_acc'] = data['val_acc']
        results[k] = data
...
gen_keys = [k for k in MODEL_ORDER if k in results and MODEL_GROUPS[k] == 'General']
fgvc_keys = [k for k in MODEL_ORDER if k in results and MODEL_GROUPS[k] == 'FGVC']
all_keys = gen_keys + fgvc_keys

models_list = [MODEL_NAMES[k] for k in all_keys]
test_acc = [results[k]['test_acc']*100 for k in all_keys]
val_acc = [results[k]['best_val_acc']*100 for k in all_keys]
...
for i, k in enumerate(all_keys):
    if i == len(gen_keys):  # Start of FGVC group
        offset += gap
...
b1 = ax.bar(x_adjusted - w/2, val_acc, w, label='Validation', color='#90CAF9', edgecolor='white', linewidth=0.5)
b2 = ax.bar(x_adjusted + w/2, test_acc, w, label='Test (Tight)', color='#1565C0', edgecolor='white', linewidth=0.5)
...
# Group labels
mid_gen = np.mean(x_adjusted[:len(gen_keys)])
mid_fgvc = np.mean(x_adjusted[len(gen_keys):])
y_label = 89.5
ax.text(mid_gen, y_label, 'General', ha='center', fontsize=10, fontstyle='italic', color='#555')
ax.text(mid_fgvc, y_label, 'FGVC-Specialized', ha='center', fontsize=10, fontstyle='italic', color='#555')
...
for k in MODEL_ORDER:
    ...
    f1_data.append(row)
    f1_model_labels.append(MODEL_NAMES[k])

f1_arr = np.array(f1_data)
fig, ax = plt.subplots(figsize=(10, 6.5))  # Taller for 8 rows
sns.heatmap(f1_arr, annot=True, fmt='.2f',
            cmap='RdYlGn', vmin=0.85, vmax=1.0,
            xticklabels=CLASSES,
            yticklabels=f1_model_labels,
            ax=ax, linewidths=0.5, cbar_kws={'label': 'F1 Score'})
ax.set_title('Per-Class F1 Score', fontsize=14, pad=12)
ax.set_xticklabels(CLASSES, rotation=0, fontsize=10, fontfamily='monospace')
ax.set_yticklabels(f1_model_labels, rotation=0, fontsize=10)

# Add horizontal line to separate General vs FGVC groups
n_gen = sum(1 for k in MODEL_ORDER if k in results and MODEL_GROUPS[k] == 'General')
ax.axhline(y=n_gen, color='black', linewidth=2)
...
# Top row: General architectures
for i, k in enumerate(MODEL_ORDER[:5]):  # General
    if k not in results: continue
    h = results[k]['history']
    ep = range(1, len(h['train_acc'])+1)
    axes[0,0].plot(ep, [a*100 for a in h['train_acc']],
             color=COLORS[i], label=MODEL_NAMES[k], linewidth=1.8, marker=MARKERS[i],
             markevery=3, markersize=4)
    axes[0,1].plot(ep, [a*100 for a in h['val_acc']],
             color=COLORS[i], label=MODEL_NAMES[k], linewidth=1.8, marker=MARKERS[i],
             markevery=3, markersize=4)

# Bottom row: FGVC methods
for i, k in enumerate(MODEL_ORDER[5:]):  # FGVC
    if k not in results: continue
    h = results[k]['history']
    ep = range(1, len(h['train_acc'])+1)
    ci = i + 5  # color index
    axes[1,0].plot(ep, [a*100 for a in h['train_acc']],
             color=COLORS[ci], label=MODEL_NAMES[k], linewidth=1.8, marker=MARKERS[ci],
             markevery=3, markersize=4)
    axes[1,1].plot(ep, [a*100 for a in h['val_acc']],
             color=COLORS[ci], label=MODEL_NAMES[k], linewidth=1.8, marker=MARKERS[ci],
             markevery=3, markersize=4)

for row, title_prefix in enumerate(['General Architectures', 'FGVC-Specialized Methods']):
    for col, metric in enumerate(['Training Accuracy', 'Validation Accuracy']):
        ax = axes[row, col]
        ax.set_xlabel('Epoch', fontsize=12)
        ax.set_ylabel('Accuracy (%)', fontsize=12)
        ax.set_title(f'{title_prefix} — {metric}', fontsize=12)
        ax.legend(fontsize=9, loc='lower right')
        ax.set_ylim(0 if row == 1 else 80, 101)  # PMG starts lower
...
paths, labels_arr = [], []
for p in sorted(glob.glob('train_small/*.jpg') or glob.glob('train/*.jpg')):
...
if len(paths) == 0:
    print('  [SKIP] No images found, skipping embedding figures (6-8)', flush=True)
...
for fname in ['difficulty_tiers.pdf', 'model_comparison.pdf', 'f1_heatmap.pdf',
              ...
              'weight_evolution.pdf', 'mantel_null_distribution.pdf']:
    src = f'{FIG_DIR}/{fname}'
